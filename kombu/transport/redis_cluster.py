...
=====================
Redis Clster transport.
Broker format:
redis-cluster://127.0.0.1:30001/0?alts=127.0.0.1:30002,127.0.0.1:30003
"""
...
from rediscluster.connection import ClusterConnectionPool, ClusterConnection
from rediscluster.exceptions import MovedError, RedisClusterException


logger = get_logger('kombu.transport.redis_cluster')
...
def Mutex(client, name, expire):
    ...
    try:
        ...
        if i_won:
            try:
                with client.pipeline(False) as pipe:
...
class MultiChannelPoller(object):
    ...
    def _register(self, channel, client, conn, cmd_type):
        if (channel, client, conn, cmd_type) in self._chan_to_sock:
            self._unregister(channel, client, conn, cmd_type)
        if conn._sock is None:   # not connected yet.
            conn.connect()
        sock = conn._sock
        self._fd_to_chan[sock.fileno()] = (channel, conn, cmd_type)
        self._chan_to_sock[(channel, client, conn, cmd_type)] = sock
        self.poller.register(sock, self.eventflags)

    def _unregister(self, channel, client, conn, cmd_type):
        self.poller.unregister(
            self._chan_to_sock[(channel, client, conn, cmd_type)])
    ...
    def _register_BRPOP(self, channel):
        ...
        if not self.socks:
            ...
            keys = {
                channel._q_for_pri(queue, 0)
                # for pri in PRIORITY_STEPS
                for queue in queues
            }

            conns = [
                client.connection_pool.get_connection_by_key(key)
                for key in keys
            ]

        else:
            conns = [conn for _, _, conn, _ in self.socks]

        for conn in conns:
            ident = channel, channel.client, conn, 'BRPOP'
            if (
                conn._sock is None or
    ...
    #     """enable LISTEN mode for channel."""
        ...
        #     self._register(channel, channel.subclient,
        #                    channel.subclient.connection, 'LISTEN')
    ...
    def on_readable(self, fileno):
        try:
            chan, conn, cmd_type = self._fd_to_chan[fileno]
        except KeyError:
            return
        if chan.qos.can_consume():
            return chan.handlers[cmd_type](**{'conn': conn})
    ...
    def handle_event(self, fileno, event):
        if event & READ:
            ...
            chan, conn, cmd_type = self._fd_to_chan[fileno]
            chan._poll_error(cmd_type, conn)
...
class Channel(virtual.Channel):
    ...
    def _brpop_start(self, timeout=1):
        ...
        for key in keys:
            ...
            _m.setdefault(node['name'], []).append(key)

        for chan, cli, conn, cmd_type in self.connection.cycle.socks:
            node_name = conn.node['name']
            if (
                node_name in _m and
                (chan, cli, cmd_type) == (self, self.client, "BRPOP")
            ):
                conn.send_command('BRPOP', *_m[node_name]+[timeout])
    ...
    def _brpop_read(self, **options):
        ...
        try:
            ...
            try:
                ...
                client.connection_pool.nodes.slots[e.slot_id][0] = node
    ...
    def _poll_error(self, type, conn, **options):
        if type == 'LISTEN':
    ...
    def _parse_hosts(self, hostname):

        """
        hostname:
        redis-cluster://username:password@127.0.0.1:30001/0?alts=127.0.0.1:30002'
        """
        _, host, port, _, password, path, query = _parse_url(hostname)
        ...
        if query.get('alts'):
            alts = query['alts'].split(',')

            for url in alts:
                _, host, port, _, _, _, _ = _parse_url('://' + url)
...
class Transport(virtual.Transport):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if rediscluster is None:
            raise ImportError('Missing redis library (pip install '
                              'redis-py-cluster)')
